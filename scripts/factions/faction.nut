this.faction <- {
	...
	function addContract( _c )
	{
		...
		if (this.getType() == this.Const.FactionType.Settlement)
		{
			...
			{
				local error = "";
				error = error + ("Failed to add contract: " + _c.getName() + " (category=" + cat + ") | Settlement: " + s.getName() + " (size=" + s.getSize() + ")");
				error = error + (" | Existing contracts of relevant category: " + cat + "=" + this.m.ContractsByCategory[cat].len() + ", Wildcard=" + this.m.ContractsByCategory.Wildcard.len());
	...
	function removeContract( _c )
	{
		...
		if (this.getType() == this.Const.FactionType.Settlement)
		{
			...
			{
				local error = "";
				error = error + ("Attempting to remove contract: " + _c.getName());
				error = error + (" (Category=" + _c.getCategory() + ",StoredAsWildcard=" + _c.m.Flags.get("StoredAsWildcard") + ")" + " from {" + this.getName() + "}");
				error = error + " but it could not be found in any Category";
				::Legends.Mod.Debug.printError(error, ::Const.LegendMod.Debug.Flags.ContractCategories);
				return;
			}
		}
	...
	function removeActionByID( _id )
	{
		...
		for( local startIndex = 0; !reachedEnd;  )
		{
