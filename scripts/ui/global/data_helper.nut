this.data_helper <- {
	...
	function addStatsToUIData( _entity, _target )
	{
		...
		local dm = 1.0;
		dm = dm * (_entity.isArmedWithMeleeWeapon() || _entity.getSkills().hasSkill("actives.hand_to_hand") ? properties.MeleeDamageMult : 1.0);
		dm = dm * (_entity.isArmedWithMeleeWeapon() || _entity.getSkills().hasSkill("actives.hand_to_hand") ? properties.RangedDamageMult : 1.0);
