this.legend_favoured_enemy_skill <- this.inherit("scripts/skills/skill", {
	...
	function getUnactivatedPerkTooltipHints()
	{
		...
		if (stats.Kills > 0)
		{
			killsText = this.format("This character has already made %s favored enemy kills", ::Const.UI.getColorized(stats.Kills, ::Const.UI.Color.getHighlightLightBackgroundValue()));
		}
		else
		{
			killsText = this.format("This character has not made any favored enemy kills yet");
		}
		...
		local ret = [
			...
			},
			{
				...
				text = this.format("Activating this perk will grant a %s bonus to the relevant stats", ::Const.UI.getColorized("+" + stats.HitChance + "%", ::Const.UI.Color.PositiveValue))
			}
		];
		return ret;
	}
